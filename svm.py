...
def load_ps_stats(filepath):
    ...
    vec = np.array(numbers, dtype=float)
...
X_train, y_train, type_train = shuffle(
    X_train, y_train, type_train, random_state=0)
...
delta_corr = np.abs(np.abs(corr1) - np.abs(corr2))
...
# Save CSV
# df.to_csv("test_pair_results_svm.csv", index=False, float_format="%.4f")

# print("Saved test_pair_results_svm.csv")



# df.to_csv("test_pair_results_svm_negative.csv", index=False, float_format="%.4f")

# print("Saved test_pair_results_svm_negative.csv")



df.to_csv("test_pair_results_svm_pcp.csv", index=False, float_format="%.4f")

print("Saved test_pair_results_pcp.csv")



# df.to_csv("test_pair_results_svm_pcp_negative.csv", index=False, float_format="%.4f")
