...
void write_statistics(const statsStruct stats, const paramsStruct params, int nz)
{
  ...
  fprintf(fp, "\n");
  fprintf(fp, "Skewness, kurtosis, variance, mean, maximum and minimum of the reconstructed texture\n");
  for (l = 0; l < nz; l++)
    for (i = 0; i < 6; i++)
  ...
  fprintf(fp, "\n");
  fprintf(fp, "Pairwise cross-correlations of the modulus of all oriented sub-bands at the same scale (size QxQ)\n");
  for(i = 0; i < N_pyr; i++) {
  ...
  fprintf(fp, "\n");
  fprintf(fp, "Cross-correlations of the modulus of all oriented sub-bands with all oriented sub-bands at the coarser scale (size QxQ)\n");
  for(i = 0; i < N_pyr - 1; i++) {
    for(l = 0; l < N_steer*N_steer*nz*nz; l++)
  ...
  fprintf(fp, "\n");
  fprintf(fp, "Cross-correlations of the real part of each oriented sub-band with both the real and imaginary part of all phase-doubled oriented sub-bands at the next coarser scale (Q matrices of size 1x2Q, which are computed and stored in a matrix of size Qx2Q)\n");
  for(i = 0; i < N_pyr - 1; i++) {
  ...
  if( nz == 3 ) {
    ...
    fprintf(fp, "\n");
    fprintf(fp, "Color Covariance matrix PCA\n");
    for(l = 0; l < 3; l++)
      for(i = 0; i < 3; i++)
    ...
    fprintf(fp, "\n");
    fprintf(fp, "Autocorrelations of each PCA band\n");
    ...
    fprintf(fp, "\n");
    fprintf(fp, "Skewness and kurtosis of each PCA band\n");
    for (l = 0; l < 3; l++)
      for (i = 0; i < 2; i++)
    ...
    fprintf(fp, "\n");
    fprintf(fp, "Pairwise cross-correlations of the real part of all oriented sub-bands at the same scale (size 3Qx3Q)\n");
    for(i = 0; i < N_pyr; i++) {
      for(l = 0; l < N_steer*N_steer*nz*nz; l++)
    ...
    fprintf(fp, "\n");
    fprintf(fp, "Pairwise cross-correlation of the low-frequency residual l_band zoomed by a factor 2 and its shifted versions by 2 pixels along the 4 possible directions (size 15x15)\n");
    ...
    fprintf(fp, "\n");
    fprintf(fp, "Cross-correlation of the real part of all oriented sub-bands at the coarser scale P with the 5 color images defined in (xi) (size 3Qx15)\n");
